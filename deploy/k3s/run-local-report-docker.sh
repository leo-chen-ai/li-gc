...
export REPORT_LOCAL_DATABASE_URL="$(secret DATABASE_URL | sed -E "s/@[^:/]+:5432\//@host.docker.internal:$DB_PORT\//")"
export REPORT_LOCAL_REDIS_URL="$(secret REDIS_URL | sed -E "s#@?[^/@:]+:6379#@host.docker.internal:$REDIS_PORT#")"
export REPORT_LOCAL_NATS_URL="nats://host.docker.internal:$NATS_PORT"
export REPORT_LOCAL_MQTT_URL="mqtt://host.docker.internal:$MQTT_PORT"
...
if ! docker run --rm --add-host host.docker.internal:host-gateway postgres:16-alpine \
  pg_isready -h host.docker.internal -p "$DB_PORT" -t 10 >/dev/null; then
...
echo "Local report stack is ready: http://localhost:8073"

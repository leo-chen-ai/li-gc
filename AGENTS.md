...
- 新建数据库表时，物理表名和字段名继续使用英文 `snake_case`，不得改成中文标识符。
- 每个 `CREATE TABLE` 必须在同一迁移中添加中文 `COMMENT ON TABLE`，说明该表的业务用途。
- 每个新增字段（包括建表字段和后续 `ALTER TABLE ... ADD COLUMN` 字段）必须在同一迁移中添加中文 `COMMENT ON COLUMN`，准确说明字段含义、单位或数据格式；金额、时间、枚举、密文、哈希、外部平台 ID 等字段应明确标注。
- 修改字段语义、单位或用途时，必须同步更新对应中文字段备注；删除字段时不需要保留旧备注。
- 数据库迁移完成后必须验证业务表和业务字段的备注覆盖率，不能出现新增表或新增字段备注为空。
- 回滚迁移应同步撤销该迁移新增的表备注和字段备注，保证 up/down 行为一致。

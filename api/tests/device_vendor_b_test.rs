...
#[tokio::test]
async fn legacy_supplemental_attendance_pull_routes_are_removed() {
    let (app, _pool, _container) = build_test_app_with_pool().await;
    for request in [
        Request::builder()
            .method("GET")
            .uri("/attendance-jobs?deviceId=DEVICE-B-001")
            .body(Body::empty())
            .unwrap(),
        Request::builder()
            .method("POST")

...
type StructuredWorkerSingleSelectProps = {
  workers: StructuredWorkerOption[];
  value: string;
...
/** 可复用的“单位 → 班组 → 人员”结构化单选组件。 */
export function StructuredWorkerSingleSelect({
  workers,
  value,
  onChange,
  placeholder = "搜索并选择人员",
  disabled,
}: StructuredWorkerSingleSelectProps) {
  ...
  const groups = useMemo(() => {
    ...
    visible.forEach((worker) => {
      ...
      const teams = unitMap.get(unit)!;
      if (!teams.has(team)) teams.set(team, []);
      teams.get(team)!.push(worker);
    });
  ...
  return (
    <Popover open={open} onOpenChange={(next) => { setOpen(next); if (!next) setKeyword(""); }}>
      <PopoverTrigger asChild>
        <Button type="button" variant="outline" role="combobox" disabled={disabled} className="h-auto min-h-10 w-full justify-between bg-white px-3 font-normal dark:bg-background">
          <span className={cn("flex min-w-0 items-center gap-2 text-left", !selectedWorker && "text-muted-foreground")}>
            <Users className="size-4 shrink-0 text-[#0f6b5d]" />
            {selectedWorker ? (
              <span className="min-w-0">
                <span className="block truncate font-medium text-foreground">{selectedWorker.name}</span>
                <span className="block truncate text-xs text-muted-foreground">{selectedWorker.unitName || "未分配单位"} · {selectedWorker.teamName || "未分配班组"}</span>
              </span>
      ...
      <PopoverContent align="start" className="w-[min(620px,calc(100vw-32px))] p-0">
        <div className="border-b p-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
            <Input autoFocus value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索姓名、手机号、身份证、单位或班组" className="pl-9" />
          </div>
          <div className="mt-2 text-xs text-muted-foreground">共 {workers.length} 人，当前显示 {groups.reduce((count, unit) => count + unit.teams.reduce((sum, team) => sum + team.members.length, 0), 0)} 人</div>
        </div>
        <div className="max-h-[420px] overflow-y-auto p-2">
          {groups.length === 0 ? <div className="py-10 text-center text-sm text-muted-foreground">没有找到匹配人员</div> : groups.map((unit) => (
            <div key={unit.unitName} className="mb-2 overflow-hidden rounded-lg border">
              <div className="flex items-center justify-between bg-slate-50 px-3 py-2 text-sm font-semibold dark:bg-muted/40"><span>{unit.unitName}</span><span className="text-xs font-normal text-muted-foreground">{unit.teams.length} 个班组</span></div>
              {unit.teams.map((team) => (
                <div key={team.teamName} className="border-t">
                  <div className="flex items-center justify-between px-3 py-1.5 text-xs font-medium text-muted-foreground"><span>{team.teamName}</span><span>{team.members.length} 人</span></div>
                  <div className="grid gap-1 bg-slate-50/40 p-2 sm:grid-cols-2 dark:bg-muted/10">
                    {team.members.map((worker) => (
                      <button key={worker.id} type="button" onClick={() => { onChange(worker.id); setOpen(false); setKeyword(""); }} className={cn("flex min-w-0 items-center gap-2 rounded-md border bg-white px-2.5 py-2 text-left hover:border-emerald-300 hover:bg-emerald-50/60 dark:bg-background", value === worker.id && "border-emerald-400 bg-emerald-50 dark:bg-emerald-950/30")}>
                        <span className={cn("flex size-6 shrink-0 items-center justify-center rounded-full border text-xs", value === worker.id && "border-[#0f6b5d] bg-[#0f6b5d] text-white")}>{value === worker.id ? <Check className="size-3.5" /> : worker.name.slice(0, 1)}</span>
                        <span className="min-w-0"><span className="block truncate text-sm font-medium">{worker.name}</span><span className="block truncate text-xs text-muted-foreground">{worker.description || "暂无其他信息"}</span></span>
                      </button>
                    ))}
                  </div>
                </div>
